...
public interface EventRepository extends JpaRepository<Event, Integer> {
    @Query(value = "select e.event_id, e.event_date, e.category_id, e.event_type_id, e.property_id, e.property_value, (select property_value from events where property_id = e.property_id and event_date like CONCAT(subdate(date(e.event_date), 1), '%') order by event_date DESC limit 1) as tmr from events e left join properties p on p.property_id = e.property_id left join objects o on o.object_id = p.object_id where o.object_id =:object_id order by e.event_date DESC limit 1", nativeQuery = true)
    Event getLastEventsFromObject(@Param("object_id") Integer object_id);

    @Query(value = "select e.event_id, e.event_date, e.category_id, e.event_type_id, e.property_id, e.property_value from events e left join properties p on p.property_id = e.property_id left join objects o on o.object_id = p.object_id where e.event_date = CURRENT_DATE and o.object_id like %:object_id order by e.event_id DESC", nativeQuery = true)
    List<Event> getListEvents(@Param("object_id") String object_id);

    @Query(value = "select e.event_id, e.event_date, e.category_id, e.event_type_id, e.property_id, e.property_value from events e left join properties p on p.property_id = e.property_id left join objects o on o.object_id = p.object_id where e.event_date between :date_start and :date_end and o.object_id like %:object_id order by e.event_date DESC", nativeQuery = true)
    ...
    @Query(value = "select e.event_id, e.event_date, e.category_id, e.event_type_id, e.property_id, e.property_value from events e left join properties p on p.property_id = e.property_id left join objects o on o.object_id = p.object_id where o.object_id = :object_id order by e.event_id DESC limit 5", nativeQuery = true)
    List<Event> getListEventsFromObject(@Param("object_id") Integer object_id);

    @Query(value = "select e.event_id, e.event_date, e.category_id, e.event_type_id, e.property_id, e.property_value from events e left join properties p on p.property_id = e.property_id left join objects o on o.object_id = p.object_id where o.object_id = :object_id and e.event_date between :date_start and :date_end order by event_id DESC;", nativeQuery = true)
    ...
    @Query(value = "select e.event_id, date(e.event_date) as event_date, e.category_id, e.event_type_id, e.property_id, e.property_value\n" +
            ...
            "e.event_date like CONCAT(:date, '%') and\n" +
            "e.event_date = (select max(e2.event_date)\n" +
            "                      from events e2\n" +
            "                      where date(e2.event_date) = date(e.event_date)\n" +
            "                     )", nativeQuery = true)
    ...
    @Query(value = "select e.event_id, date(e.event_date) as event_date, e.category_id, e.event_type_id, e.property_id, e.property_value\n" +
            ...
            "where\n" +
            "o.object_id = :object_id and\n" +
            "e.event_date between :start and :end and\n" +
            "e.event_date = (select max(e2.event_date)\n" +
            "from events e2\n" +
